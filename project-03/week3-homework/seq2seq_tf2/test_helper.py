...
def beam_decode_all(model, dataset, vocab, params):
    results = []
    sample_size = 20000
    for i in tqdm(range(sample_size)):
        enc_data, _ = next(iter(dataset))
        results.append(beam_decode(model, enc_data, vocab, params).abstract)
    return results
...
def beam_decode(model, batch, vocab, params):
    ...
    def decode_onestep(enc_inp, enc_outputs, dec_input, dec_state, enc_extended_inp,
        ...
        """
        context_vector, _ = model.attention(dec_state,  
                                           enc_outputs) 
        _, final_dists, dec_hidden = model.decoder(dec_input, dec_state, enc_outputs, context_vector)
